...
full_table <- cards %>% 
  ...
  mutate(power = ifelse(power < 0, 0, as.numeric(power)),
         toughness = ifelse(toughness < 0, 0, as.numeric(toughness)),
...
deckify <- function(data) {
  sample_deck <- read_delim(data, delim = "\n", col_names = FALSE) %>% 
    mutate(duplicates = str_extract(X1, "\\d{0,3}"),
           ...
           card_name = substr(X1,
                              nchar(duplicates) + 2,
                              nchar(X1) - nchar(from_set) - nchar(gatherer_id) - 3),
           X1 = NULL)
}
...
earnings <- ggplot(ggdecks,
                   aes(fct_reorder(player, total_winnings),
                       total_winnings)) +
  geom_col() +
  coord_flip() +
...
copy_full_table <- full_table %>% 
  mutate(in_tournament = ifelse(name_card %in% decks$card_name |
                                paste0(name_card, " ") %in% decks$card_name |
                                name_card %in% split_cards$first, 1, 0))
  
decks_full_table <- copy_full_table %>% 
  filter(in_tournament == 1)

# decks2 <- decks %>%
#   mutate(card_name = map(card_name, ~ ifelse(str_detect(., " $") == TRUE, substr(., start = 1, stop = nchar(.) - 1), .)),
#          card_name = ifelse(str_detect(card_name, "//") == TRUE, str_split(card_name, " // "), card_name),
#          card_name = map(card_name, ~ unlist(.)[1])) %>%
#   unnest(card_name)

decks2 <- decks %>%
  mutate(card_name = map(card_name, ~ ifelse(str_detect(., " $") == TRUE, substr(., start = 1, stop = nchar(.) - 1), .)),
         card_name = map(card_name, ~ ifelse(str_detect(., "//") == TRUE, str_split(., " // "), .)),
         card_name = map(card_name, ~ unlist(.)[1])) %>%
  unnest(card_name)

# decks %>% filter(str_detect(card_name, " $") == TRUE) %>% pull(card_name) %>% substr(start = 1, stop = nchar(.) - 1)

# decks2 %>% filter(str_detect(decks2$card_name, " $") == TRUE)

# decks2$card_name %>% tail(100)

god <- decks2 %>% 
  inner_join(decks_full_table, by = c("card_name" = "name_card")) %>% 
  select(-in_tournament)
```
...
# Change power, toughness, etc to numeric instead of character (get rid of
# stars)
...
# price ~ rarity, price ~ power + toughness + 
